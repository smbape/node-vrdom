...
describe("render-update-dev", () => {
    ...
    it("should update style property", () => {
        spyOn(console, "error"); // suppress warnings
    ...
    // ensures external DOM manipulation (ex: messy third party libraries)
    // won"t break vrdom
    it("should fix manual DOM manipulation on update", () => {
        var container = document.createElement("div");
        var domNode = vrdom.render(<div>{ "before" }<span />{ "middle" }{ "end" }</div>, container);

        // normalize DOM manipulation
        // https://developer.mozilla.org/en-US/docs/Web/API/Node/normalize
        container.normalize();

        vrdom.render(<div>{ "début" }<span />{ "milieu" }{ "fin" }</div>, container);
        expect(container.textContent).toBe("débutmilieufin");

        // appendChild DOM manipulation
        domNode.appendChild( document.createTextNode("Part 1 ") );
        domNode.appendChild( document.createTextNode("Part 2 ") );
        vrdom.render(<div>{ "before" }<span />{ "middle" }{ "end" }</div>, container);
        expect(container.textContent).toBe("beforemiddleend");

        // insertBefore DOM manipulation
        domNode.insertBefore( document.createTextNode("Part 1 "), domNode.childNodes[0] );
        domNode.insertBefore( document.createTextNode("Part 2 "), domNode.childNodes[2] );
        vrdom.render(<div>{ "début" }<span />{ "milieu" }{ "fin" }</div>, container);
        expect(container.textContent).toBe("débutmilieufin");

        // replaceChild DOM manipulation
        domNode.replaceChild( document.createTextNode("Part 1 "), domNode.childNodes[0] );
        domNode.replaceChild( document.createTextNode("Part 2 "), domNode.childNodes[1] );
        vrdom.render(<div>{ "before" }<span />{ "middle" }{ "end" }</div>, container);
        expect(container.textContent).toBe("beforemiddleend");

        // removeChild DOM manipulation
        domNode.removeChild( domNode.childNodes[0] );
        domNode.removeChild( domNode.childNodes[1] );
        vrdom.render(<div>{ "début" }<span />{ "milieu" }{ "fin" }</div>, container);
        expect(container.textContent).toBe("débutmilieufin");
    ...
    it("should warn if shouldComponentUpdate do not return a boolean", () => {
        class Component extends vrdom.Component {
            render() {
                return <div />;
            }
        ...
        expect(console.error.calls.argsFor(0)[0]).toContain(
            "Component.shouldComponentUpdate(): Expecting true or false, but got `undefined`."
        );
    });
