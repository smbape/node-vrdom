...
describe("findDOMNode-prod", () => {
    ...
    it("should not warn if used inside render", () => {
        spyOn(console, "error");
    ...
    it("should not throw on invalid argument", () => {
        [true, false, 0, [], Function.prototype, {string: "string"}].forEach((obj) => {
            expect(() => vrdom.findDOMNode(obj)).not.toThrow();
        });
    });

    it("should not throw when called outside a rendering process", () => {
        class Component extends vrdom.Component {
            render() {
                return <div />;
            }
        ...
        expect(() => vrdom.findDOMNode(instance)).not.toThrow();
    });
});